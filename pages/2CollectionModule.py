...
def get_data_from_excel():
        df = pd.read_excel(io=r"C:\Users\Sarthak\Downloads\Dashboards\Dashboards\files_updated\files\A_Score_amani.xlsx",engine="openpyxl")
        return df
...
if selected_option=='Borrower Profile':
    ...
    #img=get_image_as_base64("C:\Users\Sarthak\Downloads\Dashboards\Dashboards\background_pic.jpg")
    st.markdown(page_bg_img, unsafe_allow_html=True)
    df_selection = df.query("LoanId == @user")   
    df_selection.reset_index(inplace=True)
    data_selection=df.query("LoanId == @user")
